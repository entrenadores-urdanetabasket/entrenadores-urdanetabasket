...
export default function DirectorPage() {
  ...
  const [teamDetail, setTeamDetail] = useState({})         // { [teamId]: { players, incidents, attPct } }
  const [loadingDetail, setLoadingDetail] = useState(null) // team id loading
  ...
  useEffect(() => {
    ...
    else if (profile) { loadTeams(); loadOverview() }
  }, [profile])
  ...
  async function loadOverview() {
    ...
    if (teamIds.length > 0) {
      const { data: attData } = await supabase.from('attendance').select('team_id, status').in('team_id', teamIds).gte('date', fromDate)
      const byTeam = {}
      ;(attData || []).forEach(a => {
  ...
  async function toggleTeamDetail(teamId) {
    ...
    if (teamDetail[teamId]) return   // already loaded
    ...
    const [{ data: players }, { data: incidents }, { data: attData }] = await Promise.all([
      ...
      supabase.from('incidents').select('id, type, description, date, resolved, players(full_name)').eq('team_id', teamId).eq('resolved', false).order('date', { ascending: false }).limit(5),
      supabase.from('attendance').select('status').eq('team_id', teamId).gte('date', fromDate),
    ])

    let attPct = null
    if (attData && attData.length > 0) {
      const attended = attData.filter(a => a.status === 'present' || a.status === 'late').length
      attPct = Math.round((attended / attData.length) * 100)
    }
      ...
      <div style={{ display: 'flex', gap: 6, marginBottom: 20 }}>
        {[{ key: 'resumen', label: '📊 Resumen' }, { key: 'equipos', label: '👥 Equipos' }].map(t => (
          <button key={t.key} onClick={() => setTab(t.key)} style={{
            ...
            color: tab === t.key ? '#fff' : '#6b7280', transition: 'all 0.15s',
          }}>{t.label}</button>
        ))}
      </div>
            ...
            {overview.attendanceSummary.length > 0 && (
              <div style={{ backgroundColor: '#fff', borderRadius: 16, border: '1px solid #f3f4f6', overflow: 'hidden' }}>
                ...
                <div style={{ padding: '10px 18px', display: 'flex', flexDirection: 'column', gap: 8 }}>
                  {overview.attendanceSummary.slice(0, 8).map(t => {
              ...
              {overview.incidents.length === 0 ? (
                ...
                <div style={{ display: 'flex', flexDirection: 'column' }}>
                  {overview.incidents.map((inc, i) => {
                    ...
                    const dateStr = inc.date ? new Date(inc.date + 'T12:00:00').toLocaleDateString('es-ES', { day: 'numeric', month: 'short' }) : '—'
                    return (
                      <div key={inc.id} style={{ padding: '12px 18px', borderBottom: i < overview.incidents.length - 1 ? '1px solid #f9fafb' : 'none', display: 'flex', alignItems: 'flex-start', gap: 12 }}>
                        <span style={{ fontSize: 10, fontWeight: 700, padding: '2px 7px', borderRadius: 5, flexShrink: 0, marginTop: 1, backgroundColor: it.bg, color: it.color }}>{it.label}</span>
                        <div style={{ flex: 1, minWidth: 0 }}>
                          ...
                          <div style={{ fontSize: 11, color: '#9ca3af', marginTop: 2 }}>{inc.teams?.name}{inc.players?.full_name ? ` · ${inc.players.full_name}` : ''}</div>
                        </div>
                        <div style={{ flexShrink: 0, textAlign: 'right' }}>
                          <span style={{ fontSize: 11, color: '#9ca3af' }}>{dateStr}</span>
                          {inc.resolved && <div style={{ fontSize: 10, fontWeight: 700, color: '#16a34a', backgroundColor: '#f0fdf4', padding: '1px 6px', borderRadius: 4, marginTop: 2 }}>✓ Resuelta</div>}
                        </div>
                      </div>
                    )
                  })}
        ...
        loading ? (
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
            {teams.map(team => {
              ...
              return (
                <div key={team.id} style={{ backgroundColor: '#fff', borderRadius: 14, border: `1px solid ${isOpen ? '#d1f0d1' : '#f3f4f6'}`, boxShadow: isOpen ? '0 2px 12px rgba(82,176,67,0.1)' : '0 1px 4px rgba(0,0,0,0.04)', overflow: 'hidden', transition: 'all 0.2s' }}>

                  {/* Cabecera equipo — clicable */}
                  <div onClick={() => toggleTeamDetail(team.id)} style={{ padding: '16px 20px', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 16 }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 14, minWidth: 0 }}>
                      <div style={{ width: 44, height: 44, borderRadius: 12, flexShrink: 0, background: 'linear-gradient(135deg,#52B043,#1C5C2A)', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#fff', fontSize: 18 }}>🏀</div>
                      <div style={{ minWidth: 0 }}>
                        <div style={{ fontWeight: 700, fontSize: 15, color: '#111827' }}>{team.name}</div>
                        <div style={{ fontSize: 12, color: '#9ca3af', marginTop: 2 }}>{team.category} · {team.gender === 'femenino' ? 'Femenino' : 'Masculino'} · {team.season}</div>
                        <div style={{ marginTop: 5, display: 'flex', flexWrap: 'wrap', gap: 5 }}>
                          {team.coaches?.length > 0
                            ...
                            : <span style={{ fontSize: 11, color: '#f59e0b', fontWeight: 600 }}>⚠️ Sin entrenador</span>
                          }
                        </div>
                      </div>
                    </div>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexShrink: 0 }}>
                      ...
                      <button onClick={(e) => handleDelete(team.id, e)} disabled={deleting === team.id} style={{ padding: '6px 12px', borderRadius: 8, border: '1px solid #fecaca', backgroundColor: '#fef2f2', color: '#ef4444', fontSize: 12, fontWeight: 600, cursor: 'pointer' }}>{deleting === team.id ? '...' : '🗑'}</button>
                  ...
                  {isOpen && (
                    <div style={{ borderTop: '1px solid #f3f4f6', padding: '16px 20px', backgroundColor: '#fafafa' }}>
                      {isLoading ? (
                        <div style={{ textAlign: 'center', padding: '20px 0', color: '#9ca3af', fontSize: 13 }}>Cargando datos del equipo...</div>
                      ) : detail ? (
                        <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>

                          {/* Asistencia + resumen */}
                          <div style={{ display: 'flex', gap: 10 }}>
                            <div style={{ flex: 1, backgroundColor: '#fff', borderRadius: 10, padding: '12px 14px', border: '1px solid #f3f4f6', textAlign: 'center' }}>
                              <div style={{ fontSize: 22, fontWeight: 900, color: '#111827' }}>{detail.players.length}</div>
                              <div style={{ fontSize: 11, color: '#9ca3af', fontWeight: 600 }}>Jugadores</div>
                            </div>
                            <div style={{ flex: 1, backgroundColor: '#fff', borderRadius: 10, padding: '12px 14px', border: '1px solid #f3f4f6', textAlign: 'center' }}>
                              <div style={{ fontSize: 22, fontWeight: 900, color: detail.attPct !== null ? (detail.attPct >= 80 ? '#16a34a' : detail.attPct >= 60 ? '#d97706' : '#ef4444') : '#9ca3af' }}>
                                {detail.attPct !== null ? `${detail.attPct}%` : '—'}
                              </div>
                              <div style={{ fontSize: 11, color: '#9ca3af', fontWeight: 600 }}>Asistencia 30d</div>
                            </div>
                            <div style={{ flex: 1, backgroundColor: '#fff', borderRadius: 10, padding: '12px 14px', border: '1px solid #f3f4f6', textAlign: 'center' }}>
                              <div style={{ fontSize: 22, fontWeight: 900, color: detail.incidents.length > 0 ? '#ef4444' : '#16a34a' }}>{detail.incidents.length}</div>
                              <div style={{ fontSize: 11, color: '#9ca3af', fontWeight: 600 }}>Incidencias activas</div>
                            </div>
                          </div>
                          ...
                          {detail.players.length > 0 && (
                            <div>
                              <div style={{ fontSize: 12, fontWeight: 700, color: '#6b7280', marginBottom: 8, textTransform: 'uppercase', letterSpacing: 0.8 }}>Plantilla</div>
                              <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6 }}>
                                {detail.players.map(p => (
                                  <div key={p.id} style={{ display: 'flex', alignItems: 'center', gap: 6, backgroundColor: '#fff', borderRadius: 8, padding: '6px 10px', border: '1px solid #f3f4f6' }}>
                                    <span style={{ width: 22, height: 22, borderRadius: 5, background: 'linear-gradient(135deg,#52B043,#1C5C2A)', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#fff', fontSize: 10, fontWeight: 900 }}>{p.number ?? '—'}</span>
                          ...
                          {detail.incidents.length > 0 && (
                            <div>
                              <div style={{ fontSize: 12, fontWeight: 700, color: '#6b7280', marginBottom: 8, textTransform: 'uppercase', letterSpacing: 0.8 }}>Incidencias activas</div>
                              <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
                                {detail.incidents.map(inc => {
                                  const it = INCIDENT_TYPES[inc.type] || INCIDENT_TYPES.other
                                  return (
                                    <div key={inc.id} style={{ display: 'flex', alignItems: 'center', gap: 10, backgroundColor: '#fff', borderRadius: 8, padding: '8px 12px', border: `1px solid ${it.bg}` }}>
                                      ...
                                      {inc.players?.full_name && <span style={{ fontSize: 11, color: '#9ca3af', flexShrink: 0 }}>{inc.players.full_name}</span>}
                                    </div>
                                  )
                                })}
                              </div>
          ...
          <div style={{ backgroundColor: '#fff', borderRadius: 20, padding: 28, width: '100%', maxWidth: 480, boxShadow: '0 20px 60px rgba(0,0,0,0.15)', margin: 'auto' }}>
            ...
            <form onSubmit={handleSave} style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
              <div>
                ...
                <input type='text' value={form.name} onChange={e => setForm(f => ({ ...f, name: e.target.value }))} placeholder='Ej: Cadete A' required
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
                <div>
                  ...
                  <select value={form.category} onChange={e => setForm(f => ({ ...f, category: e.target.value }))} style={{ width: '100%', padding: '11px 14px', borderRadius: 10, fontSize: 14, border: '1.5px solid #e5e7eb', color: '#111827', outline: 'none', boxSizing: 'border-box' }}>
                    {CATEGORIES.map(c => <option key={c} value={c}>{c}</option>)}
                  </select>
                </div>
                <div>
                  <label style={{ display: 'block', fontSize: 13, fontWeight: 600, color: '#374151', marginBottom: 6 }}>Género</label>
                  <select value={form.gender} onChange={e => setForm(f => ({ ...f, gender: e.target.value }))} style={{ width: '100%', padding: '11px 14px', borderRadius: 10, fontSize: 14, border: '1.5px solid #e5e7eb', color: '#111827', outline: 'none', boxSizing: 'border-box' }}>
              ...
              <div>
                ...
                <select value={form.season} onChange={e => setForm(f => ({ ...f, season: e.target.value }))} style={{ width: '100%', padding: '11px 14px', borderRadius: 10, fontSize: 14, border: '1.5px solid #e5e7eb', color: '#111827', outline: 'none', boxSizing: 'border-box' }}>
                ...
                <div>
                  ...
                  {availableCoaches.length > 0 ? (
                    <div style={{ display: 'flex', gap: 8 }}>
                      <select value={addingCoach} onChange={e => setAddingCoach(e.target.value)} style={{ flex: 1, padding: '10px 12px', borderRadius: 10, fontSize: 13, border: '1.5px solid #e5e7eb', color: '#111827', outline: 'none' }}>
                        <option value=''>— Selecciona entrenador —</option>
                        {availableCoaches.map(c => <option key={c.id} value={c.id}>{c.full_name}</option>)}
                      </select>
                      <button type='button' onClick={handleAddCoach} disabled={!addingCoach} style={{ padding: '10px 16px', borderRadius: 10, border: 'none', background: addingCoach ? 'linear-gradient(135deg,#52B043,#3a8a2e)' : '#e5e7eb', color: addingCoach ? '#fff' : '#9ca3af', fontSize: 13, fontWeight: 700, cursor: addingCoach ? 'pointer' : 'not-allowed' }}>Añadir</button>
                    </div>
                  ) : (
              ...
              <div style={{ display: 'flex', gap: 10, marginTop: 6 }}>
                ...
                <button type='submit' disabled={saving} style={{ flex: 1, padding: '12px', borderRadius: 10, border: 'none', background: saving ? '#e5e7eb' : 'linear-gradient(135deg,#52B043,#3a8a2e)', color: saving ? '#9ca3af' : '#fff', fontSize: 14, fontWeight: 700, cursor: saving ? 'not-allowed' : 'pointer' }}>{saving ? 'Guardando...' : 'Guardar'}</button>
              </div>
            </form>
          </div>
        </div>
