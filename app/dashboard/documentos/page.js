...
function DocumentosInner() {
  ...
  useEffect(() => {
    ...
    else if (documents.length > 0) pushParams({ doc: null })
  }, [urlParams, documents])
    ...
    <div className="fade-in">
      ...
      {documents.length === 0 ? (
        <div style={{ textAlign: 'center', padding: '60px 0', color: '#9ca3af', backgroundColor: '#fff', borderRadius: 16, border: '1px solid #f3f4f6' }}>
          ...
          <div style={{ fontSize: 13 }}>{isDirector ? 'Sube el primero con el botón de arriba' : 'El director aún no ha subido documentos'}</div>
        </div>
      ) : (
