...
// Inicio real de la temporada de entrenamientos (club entero): aunque un
// equipo se creara antes en la aplicación, no cuenta como incumplimiento
...
export function computeExpectedDates(team, startStr, endStr, suspendedSet = new Set()) {
  ...
  const start = !startStr || startStr < SEASON_START_DATE ? SEASON_START_DATE : startStr
  if (start > end) return []
  const dates = []
